...
pattern_name = re.compile(r'''^([A-Z]{1}[a-z]*([-a-z\']+[A-Za-z]+)*)$''')
...
#foutpath = "/media/DataHD/baby_notes/output/"
...
def filter_task(f, whitelist_dict, filterlist_dict, nameset):
    ...
    with open(f, encoding='utf-8', errors='ignore') as fin:
        ...
        for i in range(len(re_list)):
            #print(re_list)
            for j in set(re_list[i][1].split(' ')):
                name_check.add(j.title())
            #name_check = name_check | set(re_list[i][1].split(' ').title())
            #print(name_check)
        ...
        for sent in note:
            # tmp = ' '.join(sent)
            # sent = str(pattern_number.sub('**FILTERED**',sent))
            ...
            if pattern_number.findall(sent) != []:
                for item in pattern_number.findall(sent):
                    ...
                    if pattern_date.match(item[0]) is None:
                        sent = sent.replace(item[0], '**FILTERED**')
                        screened_words.append(item[0])
            # email check
            sent = str(pattern_email.sub('**FILTERED**',sent))
            ...
            while True:
                if i >= len(re_list):
                    ...
                    if re.findall(r'\b(birth|dob)\b', text, re.I) != []:
                        sent = sent.replace(re_list[i][1], '**FILTERED**')
            ...
            if pattern_date.findall(sent) != []:
                for item in pattern_dob.findall(sent):
                    if re.findall(r'birth|dob', item, re.I) != []:
                        # replacement = re.sub(r'\d{2}[-./]\d{2}[-./]\d{4}', '**FILTERED**', item)
                        replacement = str(pattern_date.sub('**FILTERED**',item))
                ...
                # keep the number format, others will remove the special chars
                    ...
                    #or ((word[1] == 'CD' or (word[1] == 'JJ') and pattern_time.findall(word)))
                        ...
                        #   elif word_output.lower().encode("utf8") not in whitelist and stemmer.stem(word_output.lower()).encode('utf8') not in whitelist:
                            ...
                            if check_flag != 1:
                                if word_lower not in filterlist_dict and word_lower not in nameset:
                                    ...
                                    #if True:
                                    if pattern_name.findall(word_output) == [] or (pattern_name.findall(word_output) != [] and not word_output.istitle()):
                                        #print(word_output)
                                        ...
                                        if results == ():
                                            if pattern_name.findall(word_output) == [] and not word_output.isupper():
                                            #if True:
                                                results = sql.sql_query(stemmer.stem(word_output))
                                            ...
                                            #logging = logging + ' phase 3 2st sql:'+str(phase3-phase2)
                                                ...
                                                if results == ():
                                                    ...
                                                    if check_flag == 3:
                                                        screened_words.append(word_output)
                                                        corrected_words.append(word_output)
                                                        ...
                                                        print(word_output, 'length of filterlist: {}'.format(len(         filterlist_dict)))
                                                        safe = False
                                                        screens_per_file[f_name] += 1
                                                        '''
                                                    screened_words.append(word_output)
                                            ...
                                            #else:
                                                #whitelist.add(word_loweor)
                                             #   if pattern_name.findall(word_output) != []:
                                             #       word_output, name_check, screend_words, safe = namecheck(word_output, name_check, screened_words, safe)
                                            
                                                else:
                                                    '''
                                                    results = sql.name_query(word_output)
                                                    if results != ():
                                                        screened_words.append(word_output)
                                                        word_output  = "**FILTERED**"
                                                        #filter_set.add(word_lower)
                                                        ...
                                                        screens_per_file[f_name] += 1
                                                    '''
                                                    if pattern_name.findall(word_output) != [] or word_output.isupper() or word_output.istitle():
                                                        print(word_output)
                                                        word_output, name_check, screend_words, safe = namecheck(word_output, name_check, screened_words, safe)
                                                    else:
                                                    #whitelist.add(word_lower)
                                                        whitelist_dict[word_lower] = 0
                                                        print(word_output, 'length of whitelist: {}'.format(len(whitelist_dict)))
        ...
        if safe == False:
            ...
            phi_containing_records = 1
        fout = foutpath+"whitelisted_V3_UMLS_"+f_name+".txt"

        with open(fout,"w") as phi_reduced_note:
            phi_reduced_note.write(phi_reduced)
        with open("filter_summary_V3_UMLS.txt", 'a') as fout:
...
def main():
    ...
    with open("nameset.pkl", "rb") as fp:
        nameset = pickle.load(fp)
    print('length of nameset: {}'.format(len(nameset)))
    ...
    results = [pool.apply_async(filter_task,(f,)+(whitelist_dict_g, filter_dict_g, nameset)) for f in glob.glob(finpath+"*.txt")]
